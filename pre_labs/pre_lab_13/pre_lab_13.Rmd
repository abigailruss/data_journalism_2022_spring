...
**Answer** The following dataframe gives me the state name, who the state's electoral votes voted for in 2016, what percent of the population voted for Trump, the percentage of the state's small businesses that received a PPP loan and finally the average amount of money provided by PPP per small business employee in the state.
```{r}
...
**Answer** We have a scatter plot with a line going through it to show the positive relationship and correlation between getting PPP loans and voting for Trump. Remember correlation is not causation. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** There is a weaker relationship and a greater spread between the points between getting PPP loans and voting for Trump. 
```{r}
...
**Answer** This calculates the r scores for the percents of businesses receiving ppp loans compared to if they voted for Trump an for the percent of ppp per small business employee compared to if they voted for Trump. The results match the graphs above. The r score for the PPP per employee is much less positively correlated than the PPP per small business with the percent Trump. The relationship between a state's percent of voting for Trump and amount of PPP per employee is positive, but at .22 so we see the relationship isn't as strong. 
```{r}
reveal_data %>%
...
**Answer** We are seeing how much the percent of Trump votes and the percent of business receiving PPP is correlated. We see a very small P value. The lower the P value the more likely what we are seeing is real and not left up to chance. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** We are running a P test. We are seeing how much the percent of Trump votes and the percent of PPP per employee is correlated to test statistical significance. We see a larger than the previous P value.  
```{r}
...
**Answer** We are running a T test to see if there is a statistically significant difference between the averages between teh mean PPP amount per employee and the mean businesses receiving PPP. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** We are looking at the P and T values to see if the difference in loan amounts between a Trump state and a Clinton state is statistically significant. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** We are running a T test between the vote share and the percent of buesesses recieving PPP. We see that the T test is small. There is some relationship between some relationship between Trumps vote share and the amount of loans but that doesn't prove tat the Trump administration made that happen. 
```{r}
