...
**Answer** Copy the url of the graphic you published here. ----> https://github.com/abigailruss/data_journalism_2022_spring/blob/main/pre_labs/pre_lab_10/vYNrr-traditional-banks-dominate-2021-ppp-lending.heic 
...
**Answer** I am loading the maryland_ppp data and saving it as a new file called md_counties. I am then grouping the new file by the project county name and counting how many counties/lines there are in the dataframe. 
...
**Answer** I am loading the maryland_ppp data and saving it as a new file called md_counties. I am then grouping the new file by the project county name and counting how many counties/lines there are in the dataframe. To get rid of the extra dataframes I'm filtering the data by only including counties where there are more than 6 loans to count.
...
**Answer** Describe the contents of the new `fips_code` column-----> The new fips code column includes the 5-digit zip code for the county.
...
**Answer** Describe here what you see in `md_counties_with_fips` --------> Now in MD counties with fips I see the combined md_counties and md_fips. It includes the project county name, the count of loans, the state, the five digit zip code, the state name, the state code, the county code, the county again and the fips code. 
...
**Answer** Describe the changes from the previous map ---------> Instead of looking at the map of the entire country it now shows me just the map of Maryland. 
...
**Answer** Describe the changes from the previous map ----> Instead of putting the zip code, when you hover over the map I now see the county code and the number of loans distributed in that county. 
...
**Answer** Put the URL of your map here. ---> https://datawrapper.dwcdn.net/fzP8U/1/ 
