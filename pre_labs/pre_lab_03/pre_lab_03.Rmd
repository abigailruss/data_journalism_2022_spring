...
**Answer** The red warning says that there is one or more parsing issues and we should see 'problems()' for details. 

```{r}
ppp_maryland_loans <- read_csv("ppp_loans_md.csv.zip", guess_max=10)
# guess max says that you're only using the first # of rows to make decisions about what each column is. The lower the number is the higher your confidence in the data is 
...
**Answer** There were 87,927 errors.
 #There was a mismatch between what we were expecting and what we got because we made our judgements based on the first 10 rows
...
**Answer**  No it does not show any parsing errors because we let it look at all of the code. 
...
**Answer** It thinks that the amount field is a numeric value (possibly with decimal points), the name field is characters and the id column is a numeric value (possibly with decimal points).
```{r}
...
ppp_maryland_loans <- ppp_maryland_loans %>%
  #you saved the data to itself almost overwriting it 
  mutate(id = as.character(id))
#as. numeric to make it a number so in () is what you want to save it as
...
**Answer**: There were 192,959 franchieses without listing a name and there were 275 Subway franchises that applied for PPP loans. 
...
**Answer**: There were 135,073 loans where there was nothing listed for forgiveness amount. 
...
**Answer**: 135,073 loans when the answer is NA. Yes, this is the same number as for the forgiveness_amount. 
...
**Answer**: There are 135073 loans where the forgiveness_date and forgiveness_amount is NA. Yes, it is the same as the previous answers. I can conclude that if there are no records for forgiveness amounts there is no way people should be getting a forgiveness date because then something would be wrong. I can conclude that these are the same 135073 records. 
...
**Answer** It drops off in August 2020 and starts again in Jan. 2021- I'm assuming there are no loans in Oct. 2020 because the first round had ended and the second round hadn't begun yet. 
```{r}
ppp_maryland_loans %>%
  mutate(month_year_approved = floor_date(date_approved, "month")) %>%
  #does the data sugguest that the program operated the way it was supposed to 
...
**Answer** The largest loan is for 10000000	dollars and the smallest loan approved is 6 dollars. 
```{r}
...
**Answer** There are 2 loans that were under $100. 
```{r}
...
**Answer** Amount is a character field.
```{r}
...
**Answer** There are repeating rows, the name column is in all caps while the rest of them are lowercase, street address does not have an underscore in the name. 
...
**Answer** It now makes it x1_id instead which means that the code is now starting character instead of a number, which makes it a valid name in R. 
...
### Task 7: Use rename()
**Task** Run the following codeblock to use the clean_names() function from janitor to standardize column names and then use rename() to change the "x1_id" column. Add a description of what this code does to your reference notebook. #this is the rename function 
...
**Answer** The error says that there is a problem with the total_amount of the summaries column because it can't sum the total amount because the thing we are trying to sum is a character and not a number. 
...
**Answer** Amount is now a dbl instead of a character. 
...
**Answer** There was one set of duplicates for Stuart Jones LLC. 
...
**Answer** The new dataframe has 5 rows now. 
...
**Answer** It made it so that all the zipcodes are 5 digits long without any hyphens. 
...
There are still 2 misspellings of Arnold. 
...
**Answer** There is one misspelling of Arnold. 
...
**Answer** There are zero misspellings of Arnold. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
