...
**Answer** Group by the counties then counting the number of loans in each county then arrange it in decending order and then giving me the top 10 counties with the most loans. 
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** I see a bar chart with an assigned x and y axis.
...
**Answer** Now the graph is in acceding order because I reordered the graph.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** Now the axis is switched so I can now see the whole name of the counties. The graph is now in descending order.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** Theme minimal gets rid of the heavy grey background. The background color and line color are now inverted so you can see the lines more clearly and you get rid of the dark background. 
...
**Answer** Now the background is a different color. The font is bigger for the county names and the color is different on the words too. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** Now we have a title but the axis labels are mixed up. On the x axis the label is now county name but the markers are still numbers. On the y axis we have the label total loans but the markers are all of the county names. We're also putting on a source line to attribute the information. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** I see a typical dataframe with the dates approved and the number of total loans approved on each date. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** I see a line graph with total loans on the y-axis and date approved on the x-axis. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Now there are many more date labels on the x-axis.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Now you can read all of the dates because the x-axis labels are now at an angle instead of going over eachother. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
