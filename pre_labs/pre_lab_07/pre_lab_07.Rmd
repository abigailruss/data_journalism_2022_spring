...
**Answer** It reads the URL and takes the contents within it. I'm not getting a dataframe, instead I'm getting a list of the two HTML tags.
...
**Answer** It saved the 6 HTML tables that were on the webpage. 
...
**Answer** Instead of all 6 tables we just have one dataframe.
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** I'm saving it as employment info. 
...
**Answer** I'm only getting one row and 2 columns. We can also now refer to the value using the dollar sign. 
...
**Answer** I get a new list of the sectors under the values tab. 
```{r}
...
**Answer** It creates a value of sector and gives me a list of each of the secotors. 
```{r}
...
**Answer** I am grabbing one table/one sector and grabbing the one data frame. 
...
**Answer** We're adding the sector code and name to each table so we know 
