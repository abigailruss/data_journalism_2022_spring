...
**so you can say businesses in the sector are employed etc. and see if there's fraud based on if the industry fits in a state
...
**Answer** The web page has a list of all of the two digit NIACS codes and what those sector codes stand for. 
...
**Answer** I see the HTML that codes the webpage with all the content and formatting. 
...
**Answer** I'm getting a new nested list for naics_industry that reads the URL and takes the contents within it. I'm not getting a dataframe, instead I'm getting a list of the two HTML tags for head and body.
...
**Answer** I see two HTML tags for the head tag and body tag. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** I see all of the tags that create the table NAICS Sectors and that the id for the table is NAICS Sectors.The information is inside an html `<table>` that has a header row and column names and rows for each sector code.
...
**Answer**naics_industry is now a list of 6 tibl tables. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** Now NAICS industry is now a list of those six HTML tables so we can find the one we want (the second table).
...
**Answer** When I click on the icon to the left it opens up the entire dataframe, so if I clicked on all 6 of the scroll icons I would see 6 different tables. The first dataframe has 4 rows and 3 columns for conversion, reference month and date released.  
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** There are 21 rows listing all the different sectors and their divisions.
...
**Answer** Instead of all 6 tables we just have one dataframe that includes all of the information from the second table.
...
**Answer** I cleaned up the dataframe get rid of the 21st row. We also have lowercase column headers. 
...
**Answer** I cleaned up the dataframe get rid of the 21st row and I have lowercase column headers.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It's a BLS website that has employment for NAICS 22 which is the utilities sector. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** I'm getting a new nested list for employment_info that reads the URL and takes the contents within it. I'm not getting a dataframe, instead I'm getting a list of the two HTML tags for head and body.
```{r}
...
**Answer** I'm reading the URL and then I'm finding a specific element on the page. We ask it to go and find an HTML element with a certain ID and grab it so I get only that specific table.
...
**Answer** I'm taking the information that I had before and putting it into a pretty table dataframe with 6 rows and 6 columns. 
...
**Answer** I'm only getting one row and 2 columns because I sliced. I also cleaned the names so everything is now lowercase. We can also now refer to the value using the dollar sign. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It's a BLS website that has employment for the Mining, Quarrying and Oil and Gas Extraction sector, which is NAICS 21. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It's a BLS website that has employment for the Construction sector, which is NAICS 23. 
...
**Answer** By printing the different sectors over and over I now have a list of all of the secotrs. 
```{r}
...
**Answer** I get a new list of the sectors under the values tab and its called list_of_sectors. 
```{r}
...
**Answer** It creates a value of sector and gives me a list of each of the sectors. It looks like the same list as before but its much less code to make. 
```{r}
...
**Answer** I get an error message that says "Error in sector_list : object 'sector_list' not found" because theres nothing called sector_list its just list_of_sectors.
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** There is a new list of the sectors in a dataframe called X.
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** There is an error here because I'm trying to find something called sector_name which doesn't exist because thats not the name I definied it as. 
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** I have a list called number that lists numbers 1 to 10.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer** I am seeing my dataframe for naics_industry with 20 rows and 2 columns. 
...
**Answer** It takes the Naics data frame and counts the number of rows.
...
**Answer** I now have a list called row_number that prints my list of the number of NAICS codes 1 to 20. 
...
**Answer** I am grabbing one table/one sector and grabbing the one data frame and I have one row because I've sliced off the other lines. 
...
**Answer** I am programming a URL to make it so that each sector can fit into the URL. I'm building a URL so that for each row in the dataframe has its own URL using the sector name. So now on my naics_industry I now have a URL for each industry.The table id corresponds to the sector number.
...
**Answer** Now I have an xpath Id on my naics_industry dataframe.
...
**Answer** We now have 19 rows because we got rid of the public administration sector. 
...
**Answer** We're running the website, finding the xpath id and then grabbing the data in the table and we're doing that 19 times so we need to store it somewhere. So now it looks like I'm saving each row as its own data frame. 
...
**Answer** I now have the URL and xpath for each sector web page in one list. 
```{r}
...
**Answer** I made the above list into 19 separate tables. 
...
**Answer** I cleaned up all of the individual 19 tables so that all the headers are in lowercase etc. 
...
**Answer** We're adding the sector code and name to each table so we know what industry sector the employment numbers are.
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer** I'm parsing the number and then renaming it to have shorter names and selecting the sector, description and the number of employees. 
...
**Answer** I set up an empty dataframe and each time I assemble the employment information for each sector I'm going to bind the rows so that they're going in the empty dataframe. Now I see that the 19 different tables are just one dataframe with 19 different rows with a sector, a description and employees from November 2021. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** I see the BLS website for workforce statistics for the 21st sector. It shows the unemployment rate of the past 4 months as well as back data and the seasonally adjusted employment rate. It also shows how many people were employed in thousands. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**I see the BLS website for workforce statistics for the 11th sector. It shows the unemployment rate of the past 4 months. There is no information here on the number of workers, unlike all of the other sector's pages. 
...
**Answer** I removed the data from sector 11- Agriculture, Forestry, Fishing and Hunting from my chart. 
```{r}
