...
**Answer** This creates a dataframe with information on the tweets about COVID. There are columns for the usernames of the Twitter accounts, the content of the tweet, when it was tweeted and what is the branch, state, district, title, name, gender and party of the politician who tweeted it. 
...
**Task** Run the following code. --- get rid of links
...
**Answer** The unique function reduces the things in that list to only the unique ones to get rid of repeats. 
...
**Answer** It gets a list of unique words. 
...
**Answer** It counts all the words, sorts all the words and shows me the top 25 words used in the tweets. It's interesting that a lot of the top words are filler words. (They're junk).

twarc pulls all the tweets you want
...
**Answer** Yes these words are more interesting because we have gotten rid of the boring stop words. It's interesting because the percentages are all small because they should be small, they're not stop words. 
...
**Answer** We are creating top 10 lists of unique words for both 2020 and 2021 to compare the two. 
...
**Task** Run the following code and describe the results.
**Answer** We are creating a dataframe with the results for the 10 two word phrases tweeted in 2020 by the politicians. Public health and health care is very important. Also covid releif packages and town hall. 
...
**Task** Run the following code and describe the results.
**Answer** Here again we have a dataframe with the top 10 two word phrases tweeted by politicians but this time its for 2021. Before it was healthcare and now the top 2 word phrases are vaccines. We can see changes in terminology over time. 
...
**Task** Run the following code and describe the results.
**Answer** Now I can see the top 10 two word phrases in 2020 but this time the top phrases are separated by Democrats and Republicans so we can compare the two. The democrats used health care, public health and even social distancing among the top 10. Republicans did not have social distancing but they did have american people. Democrats are tweeting about the pandemic much more than Republicans based on the results in the n column. 
...
**Task** Run the following code and describe the results.
**Answer** This is giving me a dataframe of the covid related tweets with house Republican candidates that were tweeted. In 2020 top tweets include, president Trump, nursing homes, god bless, social distancing.  In 2021 when Trump left the top tweets switched to Joe Biden, covid positive (which was number four), illegal immigrants and more. 
...
**Answer** It asks the computer to make sure the tone/sentiments of the word are correct. It assigns words positive and negative sentiment. This dataframe loads 2 dataframes, one for 2020 and one for 2021. Each dataframe has the list of the unique words and the number of times they were used and if the word is positive or negative sentiment. The sentiment for the word positive could have been incorrect because if we're talking about testing positive, the sentiment for that should be negative, but here its classified as positive. 
